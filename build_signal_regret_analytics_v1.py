...
# ============================================================================
# MERDIAN - Build Signal Regret Analytics V1
# ----------------------------------------------------------------------------
# Purpose:
#   Aggregate signal_regret_log_v1 into analytics by (symbol, direction_bias).
#
# Reads from:
#   public.signal_regret_log_v1
#
# Writes to:
#   public.signal_regret_analytics_v1
#
# Notes:
#   - This is the first analytics layer for DO_NOTHING evaluation.
#   - Works even when 30m / 60m fields are sparse.
#   - Rates are computed only from non-null regret labels for that horizon.
# ============================================================================
...
def print_header() -> None:
    print("=" * 72)
    print("MERDIAN - Build Signal Regret Analytics V1")
    print("=" * 72)
...
def fetch_regret_rows() -> List[Dict[str, Any]]:
    rows = supabase_get(
    ...
    print(f"[INFO] Regret rows fetched: {len(rows)}")
    return rows
...
def build_analytics_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for (symbol, direction_bias), items in grouped.items():
        ...
        spot_15 = [
            x for x in (parse_float(r.get("spot_move_15m_pct")) for r in items)
            if x is not None
        ]
        spot_30 = [
            x for x in (parse_float(r.get("spot_move_30m_pct")) for r in items)
            if x is not None
        ]
        spot_60 = [
            x for x in (parse_float(r.get("spot_move_60m_pct")) for r in items)
            if x is not None
        ]

        fut_15 = [
            x for x in (parse_float(r.get("futures_move_15m_pct")) for r in items)
            if x is not None
        ]
        fut_30 = [
            x for x in (parse_float(r.get("futures_move_30m_pct")) for r in items)
            if x is not None
        ]
        fut_60 = [
            x for x in (parse_float(r.get("futures_move_60m_pct")) for r in items)
            if x is not None
        ]

        labels_15 = [
            str(x) for x in (r.get("regret_label_15m") for r in items)
            if x is not None and str(x).strip()
        ]
        labels_30 = [
            str(x) for x in (r.get("regret_label_30m") for r in items)
            if x is not None and str(x).strip()
        ]
        labels_60 = [
            str(x) for x in (r.get("regret_label_60m") for r in items)
            if x is not None and str(x).strip()
        ]
        ...
        analytics_row = {
            ...
            "source": "build_signal_regret_analytics_v1",
            "raw": {
...
def main() -> int:
    ...
    rows = fetch_regret_rows()
    if not rows:
        print("[DONE] No regret rows exist yet.")
        return 0
