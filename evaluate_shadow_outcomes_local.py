...
def fetch_shadow_rows(limit: int = 200) -> List[Dict[str, Any]]:
...
def build_outcome_row(shadow_row: Dict[str, Any], require_min_horizon_minutes: int = 15) -> Optional[Dict[str, Any]]:
    symbol = str(shadow_row.get("symbol"))
    ts = parse_ts(shadow_row.get("ts"))
    shadow_created_at = parse_ts(shadow_row.get("created_at"))
    ...
    if not spot_rows:
        return {
            ...
            "shadow_created_at": shadow_created_at.isoformat() if shadow_created_at else None,
            "entry_spot": None,
            "spot_5m": None,
            "spot_15m": None,
            "spot_30m": None,
    ...
    entry_spot = find_price_at_or_after(spot_rows, ts)
    ...
    horizon_5m_available = has_horizon(spot_rows, target_5m)
    horizon_15m_available = has_horizon(spot_rows, target_15m)
    horizon_30m_available = has_horizon(spot_rows, target_30m)
    horizon_60m_available = has_horizon(spot_rows, target_60m)
...
def main() -> int:
    print("========================================================================")
    print("MERDIAN - evaluate_shadow_outcomes_local_v2")
    print("========================================================================")

    raw_shadow_rows = fetch_shadow_rows(limit=200)
    print(f"Fetched raw shadow rows: {len(raw_shadow_rows)}")

    shadow_rows = dedupe_shadow_rows_latest(raw_shadow_rows)
    print(f"Deduped shadow rows: {len(shadow_rows)}")
    ...
    for shadow_row in shadow_rows:
        ...
        print("------------------------------------------------------------------------")
        print(f"Processing {symbol} @ {ts} | shadow_created_at={created_at}")

        out = build_outcome_row(shadow_row, require_min_horizon_minutes=15)
        if out is None:
            print("No outcome row produced.")
            continue
    ...
    if not out_rows:
        print("No outcome rows prepared. Exiting with code 1.")
        return 1

    print("------------------------------------------------------------------------")
    print("Writing rows to public.shadow_outcomes_v2 ...")
    inserted = supabase_insert("shadow_outcomes_v2", out_rows)
    print(f"Inserted rows returned by Supabase: {len(inserted)}")
    print("EVALUATE SHADOW OUTCOMES V2 COMPLETED")
    return 0
