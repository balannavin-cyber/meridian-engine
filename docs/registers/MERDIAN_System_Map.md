...
| `breadth_indicators_daily` | `build_breadth_indicators_daily_local.py` (NOT `ingest_equity_eod` — corrected S67) | upsert on_conflict=universe_id,ticker,trade_date; DMAs computed from ~120d of `equity_eod` closes; invoked by `run_equity_eod_until_done.py` as DAILY_REBUILD_SCRIPT | ACTIVE (S67 — cron'd via `40 10` AWS EOD chain; unfrozen 06-04→07-06 after the S67 loop-fix) |
...
| 2026-07-04→07 | Session 65 | **OAuth credential rotation APPLIED + breadth/`equity_eod` thread resolved + carries teed up.** Google OAuth `client_secret` (leaked in the S64 nginx-config dump) rotated by creating a NEW client → changes BOTH `client_id`+`client_secret`; `/etc/oauth2-proxy.cfg` updated on `i-0878c118835386ec2`, `oauth2-proxy` `active (running)` PID 710516 @ 2026-07-04 08:05:42 UTC (old-client delete + non-allow-list denial test pending; ENH-117 security fold). Breadth/`equity_eod`: 07-03/07-06 absence = **Dhan-side EOD publish-lag, NOT a bug** — `compute_date_window()` confirmed correct (T−1 lookback, 220-day; on 07-07 the window includes 07-03/07-06; no lower-bound pin/no cursor exclusion); self-heals on the next EOD run; universe achievable ceiling grounded ~**1,159 active** (not 1,385), `COMPLETE_EOD_THRESHOLD_PCT=95` clears against the active universe; DMA fresh to 07-02 rebuilt clean at 100% active. **TD-S65-NEW-1** filed (`check_eod_coverage_freshness.py` mis-tuned → `/1` false-OK; guard-correctness, display-not-gate). Staged: 5 Lovable prompt docs reproduced+homed `docs/lovable_prompts/` (the S64 “3” count corrected to 5) + `sql/2026-07-03_enh116_console_anon_grants.sql`; intraday-drift banner prompt ready (deploy pending). 0 confirmed commits; 0 new ADR; 0 TDs closed. See `merdian_reference.json` v44 S65 change_log. |

*MERDIAN System Map — established Session 23, 2026-05-09. Last updated Session 67, 2026-07-10 (§S67 — breadth/DMA freeze RESOLVED; the S66 "unidentified/decoupled builder" finding CORRECTED [writer IS `build_breadth_indicators_daily_local.py`, invoked by `run_equity_eod_until_done.py`; true cause = Local retired ~Jun 7 per ADR-006 leaving the canonical `40 10` AWS EOD cron self-aborting on a builder rc=1 loop-break at L182]; fix = builder rc non-fatal, verified DMAs rebuilt 06-04→07-06 @ 1,355/97.83%; three defects hardened [read-at-use token, recency floor, denominator = 1,385 active not 1,159 window-peak, true ceiling 97.83%]; commit `f5b9afd`; L204 writer attribution corrected). Previous Session 65, 2026-07-04→07 (S65 — OAuth credential rotation APPLIED [new Google client; oauth2-proxy active(running); old-client delete + denial test pending] + breadth/`equity_eod` thread resolved [07-03/07-06 = Dhan EOD publish-lag, NOT a bug; `compute_date_window` T−1/220 proven correct; universe ceiling ~1,159 active — NOTE corrected S67 to 1,385 active/97.83% ceiling] + TD-S65-NEW-1 filed [`check_eod_coverage_freshness.py` mis-tuned] + 5 Lovable prompt docs reproduced/homed + intraday-drift banner prompt-ready). Previous Session 64, 2026-07-04 (§S64 — ENH-116 Ambient Environment Intelligence built + deployed end-to-end [tables `market_environment_snapshots` + `expiry_outcomes` + view `v_expiry_base_rates`; four writers; three AWS crons] + Marketview v5 six-page terminal + public TLS/Google-auth domain marketview.meridianalpha.in). Previous Session 63, 2026-07-02 (§S63 — SENSEX `compute_flip_level` fix deployed `dc63bb3` + ENH-115 P1 FII/DII participant-positioning writer/backfill/cash/cron LIVE + ENH-07 A Phase-2 commits documented). Previous Session 54, 2026-06-16 (S53–S54 crontab/orchestration recovery + ingest `*/5` + first-full-day audit isolating SENSEX compute under-write TD-S54-NEW-1); previous Session 40, 2026-05-29 (TD-S37-01 closed via view-tau patch + Marketview v4 atomic-card redesign + AWS deploy pipeline established + `v_max_pain_by_strike` view created + `update_parameter` RPC `chk_valid_from_to` bug DISCOVERED+FIXED same-session via `valid_to DROP DEFAULT`). Previous: Session 36, 2026-05-25 (TD-S30-CANDIDATE-1 closed-misdiagnosis + ENH-99 SHIPPED capture-layer resilience). Updated inline per Doc Protocol v4 Rule 1 + Rule 9.1. Source authority: `merdian_reference.json` for canonical file paths and statuses; this Map for human-readable architectural narrative and pipeline ordering.*
...
## S67 (2026-07-10) — breadth/DMA freeze RESOLVED; the S66 write-path finding CORRECTED

**The S66 "two decoupled chains / separate unidentified builder / frontier-stuck" framing above (§S66) was materially WRONG on three points. Corrected here; the §S66 text is retained per no-crunch as the diagnostic record.**

1. **The DMA writer is `build_breadth_indicators_daily_local.py`, not "unidentified" and not an independent orphan.** It upserts `breadth_indicators_daily` (on_conflict=universe_id,ticker,trade_date), computing each ticker's DMA10/20/40 from ~120 days of `equity_eod` closes. Earlier greps missed it only because the table-name string and the word "upsert" never share a line. It is **invoked by `run_equity_eod_until_done.py`** as `DAILY_REBUILD_SCRIPT` (gated behind `ingest_rc==0`), and it reads `equity_eod` from Supabase with **no Dhan token** (so it was never a token-strand victim). (Corrects L204 + §S66 bullet 2 + TD-S66-NEW-4.)

2. **The freeze was NOT caused by "decoupling."** It IS true that re-running the *candle* ingest alone doesn't rebuild DMAs — but the builder runs in the *same loop* as the ingest. The real cause: **Local was retired at AWS migration (~Jun 7; all 22 MERDIAN Local scheduled tasks correctly Disabled per ADR-006 — do NOT re-enable).** The canonical `40 10 * * 1-5 run_equity_eod_until_done.py >> logs/eod.log` AWS cron existed all along and the cron daemon fires (sentinel-tested: `cron-alive` landed within the minute) — but every scheduled run **self-aborted at lap 1**: on a cold day the cursored ingest (50 tickers/lap) hadn't filled `equity_eod` to the builder's 95%-of-universe gate, the builder raised `RuntimeError`→rc=1, and `run_equity_eod_until_done.py` **L182 broke the whole loop on that rc**, truncating the sweep before the cursor drained. `logs/eod.log` last-written Jul 7 12:38 (last manual completion). Table sat frozen at 06-04 (the last date that ever cleared 95% before the loop began self-aborting).

3. **`equity_eod.trade_date` exists** (`date` type, confirmed via `information_schema`) — the S66 "hallucinated column" halt was itself the error. And **TD-S66-NEW-5 "frontier stuck" was not a bug**: 07-07 was simply unfilled (climbed 28%→94% as this session's sweep laps + an unattended writer filled it); the 95% gate correctly held DMAs at the last complete date. Dhan 1–3 day EOD publish lag + no-sweep-since-Jun-7.

**Fixes shipped (commit `f5b9afd`, all Canon-v3 with `_PRE_S67` backups):**
- `run_equity_eod_until_done.py` L182-188 — builder rc!=0 now **non-fatal** (log + continue); only ingest failure / cursor==0 end the loop. The builder succeeds idempotently on the lap the day crosses 95%. **Verified: 14-lap sweep cursor 700→0, DMAs rebuilt 06-04→07-06 at 1,355 rows / 97.83%.**
- `build_wcb_snapshot_local.py` `build_daily_map` — 3-trading-day recency floor (`RECENCY_FLOOR_TRADING_DAYS` + `_weekdays_back()`); stale-DMA tickers → `missing_daily` (TD-S66-NEW-2).
- `check_eod_coverage_freshness.py` — denominator = `_active_universe_count()` (live `dhan_scrip_map` = **1,385 active**, NOT the ~1,159 window-peak); window-peak → sanity cross-check; min-coverage 95→92 (true ceiling **97.83%**, ~28 DH-905 dead ids + 2 no-history) (TD-S66-NEW-3). Live run RESULT OK exit 0.
- `ingest_equity_eod_local.py` — token read-at-use (`_current_dhan_token()` re-loads `.env` per fetch, L113); verified live RELIANCE auth 200 (TD-S66-NEW-1).

**Corrected universe figures:** active equity universe = **1,385** (dhan_scrip_map is_active + dhan_security_id not null), NOT ~1,159 (which was the guard's trailing-window peak, ~200 low). True EOD coverage ceiling = **97.83%**, structural (DH-905 dead tail), not a shortfall.
