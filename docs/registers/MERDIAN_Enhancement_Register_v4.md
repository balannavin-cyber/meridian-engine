...
| Change | Detail |
|---|---|
| ENH-35 status | SUBSTANTIALLY ADDRESSED — research series (Experiments 0-13) measured signal quality against historical data. Signal Rule Book v1.0 produced. |
| ENH-37 NEW | ICT Pattern Detection Layer — BULL_OB, BEAR_OB, BULL_FVG, JUDAS_BULL signal patterns |
| ENH-38 NEW | Remove VIX gate — replace with IV-scaled position sizing (atm_iv < 12%: 0.5x, 12-18%: 1.0x, >18%: 1.5x) |
| ENH-39 NEW | Sequence quality filter for OBs — IMP_STR skip, MOM_YES Tier 1 sizing |
| ENH-40 NEW | Relax gamma regime gate — remove for OBs/FVG, keep for JUDAS_BULL only |
| ENH-41 NEW | Dynamic exit v2 — cut losers T+30m, ride winners T+60m, half-exit on >50% gain |
| Shadow gate | Updated: 8/10 sessions complete (Apr 10 counted) |
| Summary table | Updated to reflect all new ENH-37 through ENH-41 entries |
...
*MERDIAN Enhancement Register v4 — 2026-04-11 — Living document, commit to Git after every update*
*Supersedes v3 (2026-04-09). Commit alongside session log and open items register update.*
*Supersedes v2 (2026-04-06). Commit alongside V18E appendix.*

---

## NEW Tier 1 Enhancements — From ICT Research Series (2026-04-10/11)
...
| Field | Detail |
|---|---|
| Source | ICT research series — Experiments 0–13 (2026-04-10/11) |
| Status | NOT BUILT — implementation priority |
| Dependency | None — uses existing hist_spot_bars_1m + live spot bars |
| Priority Tier | 1 |
| Commercial Relevance | Internal — core signal improvement |

**What it does:** Adds ICT geometric pattern detection to the signal engine. Four approved patterns with quantified edge:
- BULL_OB (Bullish Order Block): ATM CE. +58.2% expectancy, 81% WR. Regime-independent.
- BEAR_OB (Bearish Order Block): ATM PE. +103.4% expectancy, 86% WR. Morning kill zone best.
- BULL_FVG (Bullish Fair Value Gap): ATM CE. +23.2% expectancy, 67% WR. STRUCTURAL across all vol regimes.
- JUDAS_BULL (Opening Trap Reversal): ATM CE. +19.4% expectancy, 68% WR. LONG_GAMMA required.

**Why it matters:** Current signal engine uses gamma/breadth/momentum regime states only — no price geometry. ICT patterns add institutional candle structure as a complementary signal layer. Combined expectancy with existing regime engine exceeds either alone.

**Implementation path:** Build ICT pattern detector in Python against live `hist_spot_bars_1m` equivalent (intraday spot bars). Feed detected pattern + type into `build_trade_signal_local.py` as additional signal input. Shadow validation before live promotion.

**Research basis:** Experiments 10, 10b, 10c, 11, 12b, 13.

---

### ENH-38: Remove VIX Gate — Replace with IV-Scaled Position Sizing

| Field | Detail |
|---|---|
| Source | Experiment 5 — ATM IV stress test (2026-04-10/11) |
| Status | NOT BUILT — URGENT — current gate is destroying alpha |
| Dependency | None — atm_iv already in hist_market_state |
| Priority Tier | 1 |
| Commercial Relevance | Internal — direct P&L impact |

**What it does:** Removes `trade_allowed=False when VIX>20` gate. Replaces with continuous IV-scaled lot sizing:
- atm_iv < 12%: 0.5× lots (low vol — BULL_FVG -14.3% in this environment)
- atm_iv 12–18%: 1.0× lots (normal)
- atm_iv > 18%: 1.5× lots (elevated — BEAR_OB +174.6% vs +84.8% in normal IV)
- JUDAS_BULL: fixed 1.0× regardless (HIGH_IV degrades Judas slightly)

**Why it matters:** Experiment 5 proved the VIX gate assumption is backwards. ALL three tradeable patterns (BEAR_OB, BULL_OB, BULL_FVG) show higher option P&L expectancy in HIGH_IV (≈VIX>20) than in MED_IV (≈VIX 15-20). The gate was blocking the best trades.

**File to change:** `build_trade_signal_local.py` — remove VIX gate, add atm_iv lookup, apply lot multiplier.

**Research basis:** Experiment 5. BEAR_OB|HIGH_IV +174.6% vs BEAR_OB|MED_IV +84.8%.

---

### ENH-39: Sequence Quality Filter for OBs

| Field | Detail |
|---|---|
| Source | Experiment 8 — pre-pattern sequence detection (2026-04-11) |
| Status | NOT BUILT — implementation priority |
| Dependency | ENH-37 (ICT pattern detection must exist first) |
| Priority Tier | 1 |
| Commercial Relevance | Internal — signal quality improvement |

**What it does:** For each detected BULL_OB or BEAR_OB, computes three pre-pattern features from the 3 bars before the OB bar:
- **IMP_STR**: sum(|return|) of 3 preceding bars ≥ 0.30%. If TRUE → skip trade. OB formed mid-trend, not at exhaustion. BEAR_OB|IMP_STR = -7.4% expectancy, 50% WR.
- **MOM_YES**: ≥2 of 3 preceding bars are counter-direction candles. BEAR_OB|MOM_YES = +187% expectancy, 90% WR vs +59% without.
- **Time zone**: OPEN session (09:15-10:00) → 0.5× or skip for BULL_OB (+3.4% vs +72% morning).

**Tier sizing:**
- Tier 1 (1.5×): Morning + MOM_YES + IMP_WEK → ~298% expectancy, 100% WR
- Tier 2 (1.0×): IMP_WEK + any → +100-132% expectancy
- Tier 3 (SKIP): IMP_STR detected → -7.4% expectancy, 50% WR

**Research basis:** Experiment 8.

---

### ENH-40: Relax Gamma Regime Gate

| Field | Detail |
|---|---|
| Source | Experiments 11, 12b — regime intersection (2026-04-10/11) |
| Status | NOT BUILT |
| Dependency | ENH-37 (ICT patterns) |
| Priority Tier | 1 |
| Commercial Relevance | Internal |

**What it does:** Removes `gamma_regime = LONG_GAMMA` gate for BULL_OB, BEAR_OB, and BULL_FVG. Keeps it for JUDAS_BULL.

**Evidence:**
- BULL_OB|LONG_GAMMA: +65.7% | BULL_OB|NO_FLIP: +62.3% → regime-independent
- BEAR_OB|NO_FLIP: +124.4% | BEAR_OB|LONG_GAMMA: +19.7% → NO_FLIP actually stronger for BEAR_OB
- BULL_FVG: positive in both regimes
- JUDAS_BULL|LONG_GAMMA: +24.1% | JUDAS_BULL|NO_FLIP: -5.2% → keep gate for JUDAS

**Also relax:** breadth_regime gate for OBs (works across all breadth contexts). Keep breadth gate for JUDAS_BULL (BEARISH breadth: +47.6%, 100% WR).

**Research basis:** Experiments 11A, 11B.

---

### ENH-41: Dynamic Exit v2

| Field | Detail |
|---|---|
| Source | Portfolio simulation v2 (2026-04-10/11) |
| Status | NOT BUILT |
| Dependency | ENH-37 (ICT patterns must exist to apply exit rules to) |
| Priority Tier | 1 |
| Commercial Relevance | Internal — P&L improvement |

**What it does:** Replaces fixed T+30m exit with dynamic exit logic:
- If P&L negative at T+30m: exit all lots (cut premium decay)
- If P&L positive, gain < 50% at T+30m: hold to T+60m (winner with room to run)
- If P&L positive, gain ≥ 50% at T+30m: exit half lots at T+30m, hold rest to T+60m (lock gains, preserve upside)
- T+60m: exit unconditionally

**Evidence:** v1 vs v2 portfolio simulation:
- NIFTY Fixed: +49% → +55% (+6% improvement)
- NIFTY Pyramid: +130% → +136%
- Half-exit adds ~INR 15-20k on major trending days (May 15 2025 BULL_OB trades)

**Research basis:** Portfolio simulation v2.
