...
*CLAUDE.md v1.12 — 2026-05-03 (Session 17 close). Added: Rule 22 (direction-asymmetric defect pairs); B13 (self-patching loop anti-pattern, 2-hotfix limit); B14 (visible-console pollution); six Session 17 operational findings (check_from filter pattern, smoke must simulate cadence, N-threshold for direction-asymmetric gates, re-enable tasks after kill, holiday flashes are correctness, Pine v6 block-type unification). Settled decisions added: TD-058 BEAR_FVG live emission RESOLVED via 5-edit detector + experiment patch (BEAR_FVG signal count 0→138, +22.8pp P&L lift); TD-060 RESOLVED via runner window-slice (F4) + detector check_from removal (G1) — 100% OB coverage validated on Feb 01 NIFTY full-day smoke; ENH-91 SHIPPED Pine WR labels per pattern_type from Exp 15 cohort; ENH-92 SHIPPED Pine intraday zones merged as M5 timeframe; ENH-88 BUILT NOT DEPLOYED (waits Mon live BULL_OB data); ENH-90 CANDIDATE filed (BEAR_FVG anti-cluster -16.5pp N=22, deferred for N expansion per Rule 22 and Session 17 finding); BULL_FVG and BEAR_FVG are coin flip standalone (CIs span 50%, both must be filtered before production); cluster effect direction-asymmetric (BULL +12.8pp, BEAR -16.5pp at 90min). v1.11 (Session 16 close) Rule 21 + B11/B12. v1.10 (Session 15 close) Rule 20 + B9/B10. v1.9 (Session 14 close) Rule 18/19. v1.8 (Session 13 close) Rule 17. v1.7 (Session 11 ext). v1.6 Rules 14/15/16. v1.5 compendium-replicates. v1.4 Python path. v1.3 Rule 13. v1.2 Rule 12. v1.1 Rule 11.*
