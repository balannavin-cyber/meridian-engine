...
| ENH-02 | Put/Call Ratio (PCR) Signal | 1 | **WIRED (live, floored — S61)** |
...
| ENH-07 | Basis-Implied Risk-Free Rate | 1 | **A reframed (open) / B SHIPPED-Measure (S61)** |
...
| Status | **IN PROGRESS** (was PROPOSED — infrastructure now exists; still unbuilt — "buildable immediately"). **S57 scope add:** cost-of-carry / basis-velocity sign-read. **S61 — split A/B.** (A) basis-implied risk-free rate **REFRAMED**: there is NO live BS solver and NO live rate parameter (the live gamma engine reads vendor Dhan Greeks); the risk-free rate only matters inside the TD-S58-NEW-1 historical Greeks backfill. `core/bs_engine.py` built+validated (S=25218,K=24000,P=18,T=8/365,r=6.5%→IV=21.45%,γ=0.000134); A stays open as the historical/shadow track. (B) basis-velocity context **SHIPPED (Measure-stage, display-only)**: `compute_basis_context_local.py` (299 ln) reads `index_futures_snapshots`, writes `basis_context_snapshots` (UPSERT symbol,ts); reader floor + context-not-gate into `signal_snapshots.raw`; `hist_basis_context` backfill NIFTY 92,515 / SENSEX 29,689 (zero-shift pairing, TD-S61-NEW-2). Commit `141386d`. |
...
| 2026-06-27 | — | S61 ENH-02 wired + ENH-07 B shipped + A reframed | 3533d22 / 8ddbc78 / d16986c / 141386d | **ENH-02 PROPOSED-orphan → WIRED (live).** `compute_options_flow_local.py` re-homed into the AWS orchestrator; `build_trade_signal_local.py _fetch_options_flow` given the ADR-018 D2 recency floor `MERDIAN_FLOW_RECENCY_FLOOR_MIN`=15 after finding the ENH-02/04 ±3/4/5 confidence modifiers had run off a ~24-day-stale `options_flow_snapshots` row since the S49 migration (TD-S61-NEW-1, NEW+CLOSED). **ENH-07 split.** (A) basis-implied risk-free rate **REFRAMED** — the live gamma engine reads vendor Dhan Greeks, so there is no live BS solver and no live rate parameter; the rate only matters inside the TD-S58-NEW-1 historical Greeks backfill; `core/bs_engine.py` built+validated; A stays open. (B) basis-velocity context **SHIPPED (Measure-stage, display-only)** — `compute_basis_context_local.py` (299 ln) reads `index_futures_snapshots`, writes `basis_context_snapshots` (UPSERT symbol,ts), reader floor + context-not-gate into `signal_snapshots.raw` (`141386d`, +336); `hist_basis_context` backfill NIFTY 92,515 / SENSEX 29,689 (zero-shift pairing, TD-S61-NEW-2). No new ADR. |
