...
> **Status:** FILLED 2026-04-23 (Session 7) — seeded from AppendixV18H_v2 §7.1, Master V19 §5.1, and Session 7 incident response. **UPDATED 2026-05-14 (S29 firefighting)** — Step 2d added after partial-day outage where correctly-propagated token didn't recover because feeder held stale token in memory.
...
**If Step 2d is skipped** (added 2026-05-14), `.env` is correct but the long-running `ws_feed_zerodha.py` process holds the prior token in memory until manually killed. Symptom is identical to skipping Step 2 (empty `market_ticks` all day). Diagnostic difference: `kite.profile()` returns AUTH OK but `market_ticks` stays empty.
...
### Step 2d — Restart consumer processes (~06:08 IST) — **MANDATORY**
...
sleep 2
pgrep -f ws_feed_zerodha.py   # must return nothing

# Restart with the new token loaded fresh
cd /home/ssm-user/meridian-engine
nohup python3 ws_feed_zerodha.py >> logs/ws_feed.log 2>&1 &
disown
```

Skip this step **only** if `pgrep -f ws_feed_zerodha.py` returned nothing before the kill (no prior instance was running — the regular 09:14 IST cron will then start fresh with the new token).

If `refresh_equity_intraday_last.py` (09:05 IST cron) already fired with the broken token before you completed Step 2c, also run it manually now to refresh the breadth-reference layer immediately rather than waiting for tomorrow's cron:
...
| 09:20 IST: market_ticks zero rows AND `logs/ws_feed.log` shows `Supabase write error 500: ... statement timeout` | Distinct failure — not token. `market_ticks` table is bloated; INSERT exceeds Supabase statement_timeout (TD-NEW-A, 2026-05-14 incident) | TRUNCATE `public.market_ticks`, verify pg_cron retention job (`prune-market-ticks` jobid 46) is active, then restart feeder. |
...
| 2026-05-14 | Added **Step 2d** (consumer process restart) + two new Failure-modes rows + Additional gap. Triggered by partial-day breadth outage where correctly-propagated token didn't help because feeder held stale token. Filed as TD-NEW-F. | (S29 close commit) |
