...
*CLAUDE.md v1.11 — 2026-05-03 (Session 16 close). Added: Rule 21 (always Tee-Object long-running scripts; PYTHONIOENCODING=utf-8 for box-drawing chars); B11 (wrong-cohort comparison anti-pattern); B12 (silent code drift between publication and audit); six Session 16 operational findings (schema-first queries, confidence intervals before victory, trade-list CSV dump pattern, live-detector replay pattern, read script before conclusions, hist_pattern_signals deprecation candidate). Settled decisions added: Exp 15 framework replicates within 2-3pp on current code (₹4L→₹11.7L +193%); BULL_FVG standalone is coin flip CI [42.5,58.1]; BULL_FVG-with-recent-BULL_OB cluster is real edge (+12.8pp at 90min); MTF context hierarchy LOW > HIGH on OB (settled by Section 10 on N=231 with CIs); edge concentration top 7/57 (12.3%) = 80% of P&L is structural; Apr-13 MTF tier relabeling settled — current vocabulary (VERY_HIGH=W, HIGH=D, MEDIUM=H, LOW=none) is canonical going forward. v1.10 (Session 15 close) Rule 20 era-aware Rule 16 + B9/B10 + five findings. v1.9 (Session 14 close) Rule 18/19 + five findings. v1.8 (Session 13 close) Rule 17. v1.7 (Session 11 ext). v1.6 Rules 14/15/16. v1.5 compendium-replicates. v1.4 Python path. v1.3 Rule 13. v1.2 Rule 12. v1.1 Rule 11.*
