...
def _atm_anchor_at(sb, instrument_id: str, bar_ts_5m: datetime,
    ...
    """ENH-100 (S32) v3 — anchor ATM row at exact 5m bar_ts. Nearest expiry first.

    Uses idx_hist_atm_opt_5m_key UNIQUE (instrument_id, bar_ts, expiry_date) —
    eq+eq+ORDER+LIMIT is a direct index walk. O(log n).

    Cached per (instrument_id, bar_ts_5m). Returns dict with atm_strike,
    expiry_date, dte, ce_close, pe_close — or None if no vendor coverage.
    """
...
def _atm_future_at(sb, instrument_id: str, bar_ts_5m: datetime,
    ...
    """ENH-100 (S32) v3 — future ATM row at exact 5m bar_ts + same expiry as anchor.

    Three-eq filter against idx_hist_atm_opt_5m_key UNIQUE: direct index hit, O(log n).
    Cached per (instrument_id, bar_ts_5m, expiry).

    Same-strike enforcement happens at caller, not here — this returns whatever
    the vendor's ATM for the future bar is, caller compares against anchor's.
    """
