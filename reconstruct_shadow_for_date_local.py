...
def fetch_rows_for_ist_date(table_name: str, replay_date: str, select_cols: str = "*", limit: int = 1000) -> List[Dict[str, Any]]:
    # broad UTC window, then local IST-date filter
...
def latest_row_at_or_before(rows: List[Dict[str, Any]], symbol: str, target_ts: datetime) -> Optional[Dict[str, Any]]:
...
def latest_smdm_at_or_before(rows: List[Dict[str, Any]], symbol: str, target_ts: datetime) -> Optional[Dict[str, Any]]:
    candidates: List[Tuple[datetime, Dict[str, Any]]] = []
...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python reconstruct_shadow_for_date_local.py YYYY-MM-DD")
        return 1
    ...
    print("========================================================================")
    print("MERDIAN - reconstruct_shadow_for_date_local")
    print("========================================================================")
    print(f"Reconstruction date (IST): {reconstruction_date}")

    market_states = fetch_rows_for_ist_date("market_state_snapshots", reconstruction_date)
    momentum_rows = fetch_rows_for_ist_date("momentum_snapshots_v2", reconstruction_date)
    options_flow_rows = fetch_rows_for_ist_date("options_flow_snapshots", reconstruction_date)
    iv_context_rows = fetch_rows_for_ist_date("iv_context_snapshots", reconstruction_date)
    smdm_rows = fetch_rows_for_ist_date("smdm_snapshots", reconstruction_date)

    print(f"market_state rows: {len(market_states)}")
    print(f"momentum_v2 rows: {len(momentum_rows)}")
    print(f"options_flow rows: {len(options_flow_rows)}")
    print(f"iv_context rows: {len(iv_context_rows)}")
    print(f"smdm rows: {len(smdm_rows)}")
    ...
    for market_state in market_states:
        ...
        momentum_v2 = latest_row_at_or_before(momentum_rows, symbol, ts)
        options_flow = latest_row_at_or_before(options_flow_rows, symbol, ts)
        iv_context = latest_row_at_or_before(iv_context_rows, symbol, ts)
        smdm = latest_smdm_at_or_before(smdm_rows, symbol, ts)
        ...
        print(f"momentum_regime_v2={row['momentum_regime_v2']}")
        print(f"flow_regime={row['flow_regime']}")
        print(f"skew_regime={row['skew_regime']}")
        print(f"iv_regime={row['iv_regime']}")
        print(f"smdm_pattern={row['smdm_pattern']}")
    ...
    print("------------------------------------------------------------------------")
    print("Writing rows to public.shadow_reconstruction_v1 ...")
    inserted = supabase_insert("shadow_reconstruction_v1", out_rows)
    print(f"Inserted rows returned by Supabase: {len(inserted)}")
    print("RECONSTRUCT SHADOW FOR DATE COMPLETED")
    return 0
