...
SHADOW_POLICY_VERSION = "WCB_SHADOW_V2"
...
def _derive_shadow_from_baseline(
    ...
    # 2) Divergence handling — stronger than V1
    elif breadth_wcb_relationship in (
        "DIVERGENT_BREADTH_BULL_WCB_BEAR",
        "DIVERGENT_BREADTH_BEAR_WCB_BULL",
    ):
        shadow_confidence -= 10.0
        cautions.append("Breadth and WCB diverge")
        cautions.append("Shadow V2 applies stronger divergence penalty")

        if baseline_direction_bias in ("BULLISH", "BEARISH") and baseline_confidence_score < 75.0:
            shadow_direction_bias = "NEUTRAL"
            shadow_trade_allowed = False
            shadow_action = "DO_NOTHING"
            shadow_entry_quality = "NO_TRADE"
            cautions.append("Shadow V2 neutralized directional setup due to breadth/WCB divergence")
    ...
    # 4) Unknown / mixed states
    elif breadth_wcb_relationship == "UNKNOWN":
        cautions.append("Breadth/WCB relationship unavailable")
    else:
        cautions.append(f"Breadth/WCB relationship is {breadth_wcb_relationship}")

    # 5) Coverage penalties
    if wcb_weight_coverage_pct is not None:
    ...
    # 6) Optional confidence-based upgrade rule in shadow only
    # This only upgrades entry quality, not action, when trade is already allowed.
    if shadow_trade_allowed and shadow_confidence >= 75.0:
        reasons.append("Shadow V2 sees high-confidence setup after WCB adjustment")
    ...
    shadow_decision_changed = (
        ...
        or shadow_entry_quality != baseline_entry_quality
    )
