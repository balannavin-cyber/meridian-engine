"""
batch_backfill_market_state.py

Runs backfill_market_state.py across all historical dates.
Skips dates already present in hist_market_state.

Usage:
    python batch_backfill_market_state.py
    python batch_backfill_market_state.py 2025-10-01 2025-12-31
    python batch_backfill_market_state.py 2025-10-01 2025-12-31 NIFTY
"""
...
LOG_PATH = BASE_DIR / "logs" / "batch_backfill_market_state.log"
...
def get_done(symbol):
    rows = sb_get("hist_market_state", f"symbol=eq.{symbol}&select=trade_date&limit=5000")
    return {r["trade_date"] for r in rows}

def run_one(trade_date, symbol):
    cmd = [sys.executable, str(BASE_DIR / "backfill_market_state.py"), trade_date, symbol]
    try:
...
def main():
    ...
    log("="*72)
    log("BATCH BACKFILL MARKET STATE")
