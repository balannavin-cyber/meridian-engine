...
def _direction_from_gamma(gamma_regime: Optional[str]) -> str:
    ...
    if direction_bias == "BEARISH":
        ...
        if ret_5m is not None:
            if ret_5m < 0:
                ...
                score -= 2.0
                cautions.append("5-minute momentum is not confirming bearish bias")
        ...
        if ret_15m is not None:
            if ret_15m < 0:
                ...
                score -= 2.0
                cautions.append("15-minute momentum is not confirming bearish bias")
        ...
        if price_vs_vwap_pct is not None:
            if price_vs_vwap_pct < 0:
                ...
                score -= 2.0
                cautions.append("Price is above session VWAP")

        if atm_straddle_change is not None and atm_straddle_change > 0:
            score += 2.0
            reasons.append("ATM straddle is expanding")
        elif atm_straddle_change is not None and atm_straddle_change < 0:
            score -= 1.0

        if bearish_confirm >= 2:
            score += 5.0
            reasons.append("Momentum confirms bearish bias")
...
def _build_signal(symbol: str) -> Dict[str, Any]:
    ...
    direction_bias = _direction_from_gamma(gamma_regime)
    ...
    if india_vix is not None:
        reasons.append(f"India VIX is {india_vix:.2f}")
    if vix_regime:
        reasons.append(f"VIX regime is {vix_regime}")
    ...
    if not breadth_aligned:
        trade_allowed = False
        reasons = ["Gamma and breadth are not aligned for a directional options-buy signal"]
    ...
    if direction_bias == "BEARISH" and breadth_aligned:
        action = "BUY_PE"
    elif direction_bias == "BULLISH" and breadth_aligned:
        action = "BUY_CE"
    else:
        action = "DO_NOTHING"
    ...
    elif trade_allowed and score >= 75:
        entry_quality = "HIGH"
    elif trade_allowed and score >= 60:
    ...
    out_file = DATA_DIR / "latest_trade_signal.json"
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(signal_row, f, indent=2, default=str)
