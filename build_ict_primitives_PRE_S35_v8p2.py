...
def _load_expiry_calendar(sb, symbol: str) -> list:
    ...
    # HOCS is keyed on symbol (text), not instrument_id (uuid). Distinct expiry_date
    # over the whole table is small (~10-11 distinct per symbol per S35 audit).
    ...
    log(f"  [v8.1] expiry calendar {symbol}: vendor={len(vendor_expiries)} "
