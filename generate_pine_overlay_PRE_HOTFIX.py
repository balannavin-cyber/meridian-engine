...
draw_zone(float zlow, float zhigh, color bg, color line_col, string lbl, int look_back, bool bear_side, bool show, bool is_pdh_pdl) =>
    if barstate.islast and show
        ...
        var label_style = label_pos == "left" ? label.style_label_right : label.style_label_left

        size lbl_sz = label_size == "tiny" ? size.tiny : (label_size == "normal" ? size.normal : size.small)
