...
from outcome_engine_common import evaluate_shadow_signal_row, page_source_rows


SOURCE_TABLE = "shadow_signal_snapshots"
OUTCOME_TABLE = "shadow_signal_outcomes"


def get_existing_shadow_signal_ids(sb: SupabaseClient) -> Set[int]:
    ...
    while True:
        ...
        for row in rows:
            ...
            if shadow_signal_id is not None:
                try:
                    existing.add(int(shadow_signal_id))
                except Exception:
                    pass
...
def build_outcomes(
    ...
    print("=" * 72)
    print("MERDIAN - Build Shadow Signal Outcomes (spot timeline version)")
    print("=" * 72)
    ...
    existing_ids = set() if force_rebuild else get_existing_shadow_signal_ids(sb)
    if not force_rebuild:
        print(f"Existing shadow_signal_ids already in outcomes: {len(existing_ids)}")
    ...
    for row in source_rows:
        ...
        try:
            shadow_signal_id_int = int(shadow_signal_id)
        except Exception:
            failed += 1
            continue

        if not force_rebuild and shadow_signal_id_int in existing_ids:
            skipped_existing += 1
            continue

        outcome = evaluate_shadow_signal_row(sb, row)
        if outcome is None:
            failed += 1
            continue
