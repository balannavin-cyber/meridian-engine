-- =====================================================================
-- 2026-09-22_s81_v_max_pain_by_strike_expiry_filter.sql
-- S81 / TD-S80-NEW-10 -- expiry filter + ts + recency floor
-- =====================================================================
--
...
--   * latest_ts stays max(ts) GROUP BY symbol over the whole of
--     option_chain_snapshots -- the S72 FIX 2 cost shape. Carried
--     unchanged BY INSTRUCTION; Section 4a reports its EXPLAIN. If it
--     seq-scans, that is reported, not silently restructured inside a
--     change scoped to the expiry filter.
...
WITH latest_ts AS (
        -- UNCHANGED from the captured baseline, by instruction.
