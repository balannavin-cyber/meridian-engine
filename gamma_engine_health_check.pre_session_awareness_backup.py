...
def _overall_status(all_problems: List[str]) -> str:
    if not all_problems:
        return "HEALTHY"
...
def _print_symbol_block(now_utc: datetime, symbol: str, stage_ts: Dict[str, Optional[datetime]]) -> None:
    ...
    for table in PIPELINE_TABLES:
        ...
        print(
            f"{label:12} ts={_format_ist(ts):>12}   age={_format_age_seconds(age):>6}"
        )
...
def main():
    ...
    nifty_problems = _pipeline_problems_for_symbol(now_utc, "NIFTY", nifty_ts)
    sensex_problems = _pipeline_problems_for_symbol(now_utc, "SENSEX", sensex_ts)
    ...
    status = _overall_status(all_problems)
