"""
audit_s32_enh100_falsification_v2.py — ADR-010 v3 falsification audit (fixed).

v1 audit produced 0 DISCREPANT but also ~80% MISSING_RECOMPUTED on spot-derived
columns and 100% MISSING_RECOMPUTED on atm_pnl. Three audit-side bugs:

  1. fetch_spot_bars filter used real-UTC bounds but pre-era hist_spot_bars_1m
     bar_ts is IST-mislabeled +00 — filter never matched.
  2. spot_at compared `b_ts == probe` exactly but vendor bars are at HH:MM:59;
     probe was at HH:MM:00. Never matched.
  3. recompute_atm_pnl_from_chain used `.eq("bar_ts", ...)` against
     hist_option_bars_1m whose bar_ts is at HH:MM:59. Never matched.

v2 fixes:

  1. fetch_spot_bars now uses an IST-trading-date-bounded UTC window —
     [ist_date 00:00 UTC, next_day 00:00 UTC]. This captures both pre-era
     (IST-labeled 09:15-15:30+00 falls within the same calendar date in UTC)
     and post-era (true UTC 03:45-10:00) stored values in one filter.
     Caches per (instrument_id, ist_date) across samples for speed.
  2. spot_at uses dict {minute_floor_real_utc: bar} for O(1) probe lookup;
     bar_ts is era-normalized + minute-floored at index time.
  3. recompute_atm_pnl_from_chain uses `.gte/.lt` range filter on chain
     bar_ts: [vendor_bar_ts_label(anchor_5m), vendor_bar_ts_label(anchor_5m+1m))
     catches the HH:MM:59 stored bar.

Verified contracts from v1 run (preserved in v2):
  - dte: 51 MATCH / 0 DISC — vendor's pre-computed dte read OK
  - no DISCREPANT in any column when both sides populated → writer math correct
  - v2's additional recompute coverage will either confirm this on the
    previously-broken cohorts OR surface real writer bugs.
"""
...
# ----------------------------------------------------------------------------
# Cached spot-bar fetch (v2 fix: IST-date-bounded filter, era-normalized index)
# ----------------------------------------------------------------------------

class SpotBarsCache:
    def __init__(self, sb):
        self.sb = sb
        self._cache: dict = {}  # (instrument_id, ist_date) → {minute_floor: (o,h,l,c)}
        self.queries_made = 0

    def _fetch_ist_day(self, instrument_id: str, ist_d: date) -> dict:
        """Fetch all hist_spot_bars_1m bars for an IST trading date.

        Filter uses [ist_d 00:00 UTC, ist_d+1 00:00 UTC) which captures both
        IST-labeled +00 (pre-era; 09:15-15:30 UTC labels fall inside the same
        calendar UTC date) and true-UTC post-era (03:45-10:00 UTC) bars.
        """
        ...
        while True:
            ...
            for r in rows:
                ...
                out[key] = (float(r["open"]), float(r["high"]),
    ...
    def get_window(self, instrument_id: str, ts: datetime,
                   window_minutes: int = 150) -> dict:
        """Return merged dict of bars covering ts and ts+window. Crosses IST date
        if the window straddles."""
...
def spot_at(bars: dict, ts: datetime) -> Optional[float]:
    """Lookup close price at-or-after ts within 5-min forward window."""
...
def recompute_forward_returns(bars: dict, valid_from: datetime) -> dict:
...
def recompute_mfe_mae(bars: dict, valid_from: datetime, direction: str) -> dict:
    ...
    saw_any = False
    # Iterate over minute keys in bars dict within the window
    sorted_keys = sorted(k for k in bars if valid_from <= k <= window_end)
...
def _chain_premium_at(sb, instrument_id: str, expiry_date_v, strike: float,
                       option_type: str, target_5m: datetime) -> Optional[float]:
    """Fetch hist_option_bars_1m premium at target 5m boundary; chain bars are
    at HH:MM:59 so range-filter the 1-min window [target_5m, target_5m + 1min)."""
    start_label = vendor_bar_ts_label(target_5m)
    end_label = vendor_bar_ts_label(target_5m + timedelta(minutes=1))
    res = (
...
def recompute_atm_pnl_from_chain(sb, instrument_id: str, valid_from: datetime,
    ...
    if direction not in ("BULL", "BEAR"):
        return {}
    anchor_5m = floor_5m(valid_from)
    # Get strike + expiry the writer used (from vendor's pre-picked ATM table)
    res = (
        sb.table("hist_atm_option_bars_5m")
    ...
    premium_t0 = _chain_premium_at(sb, instrument_id, expiry, strike,
    ...
    for col, mins in [("atm_pnl_5m_pct", 5), ("atm_pnl_15m_pct", 15),
        ...
        future_premium = _chain_premium_at(sb, instrument_id, expiry, strike,
...
def audit_primitive(sb, sample, spot_cache: SpotBarsCache):
...
def main():
    ...
    print(f"[start] ENH-100 v3 falsification audit v2  n={args.n}  symbol={args.symbol}")
    sb = get_client()
    t0 = _time.time()
    ...
    for i, samp in enumerate(samples):
        ...
        if (i + 1) % 10 == 0:
            print(f"    audited {i + 1}/{len(samples)}  "
                  ...
                  f"spot_cache_days={len(spot_cache._cache)}  "
                  f"spot_queries={spot_cache.queries_made}")
    print(f"  audit completed in {_time.time() - t1:.1f}s")
    print(f"  spot bars cached for {len(spot_cache._cache)} distinct (instrument, day) pairs")
    print(f"  total spot-bar queries: {spot_cache.queries_made}")
