#!/usr/bin/env python3
"""
s30_path1_live_cohort_pnl_v3.py — Path 1 v3 with archive table routing.

v2 missed 74/81 signals because option_chain_snapshots is a rolling-window live
table; rows older than ~30 days are archived out to historical_option_chain_snapshots.
v3 routes per-signal: pre-2026-05-04 fetches from archive, post fetches from live,
with cross-table fallback on zero rows.

Schema parity confirmed: archive table has identical (ts, symbol, strike,
option_type, expiry_date, ltp) columns.

Run:
    python s30_path1_live_cohort_pnl_v3.py
"""
...
# Archive cutover: ts < this → try archive first; ts >= this → try live first
...
def fetch_chain_row_smart(sb: Client, symbol: str, strike: int, option_type: str,
                          expiry_date: str, target_ts: datetime) -> tuple[dict | None, str | None]:
    """Try archive vs live based on ts; fall back to the other table."""
    ...
    row = fetch_chain_row_from_table(sb, fallback, symbol, strike, option_type, expiry_date, target_ts)
...
def main() -> int:
    ...
    print(f"[2/2] Per-signal join (archive + live routing)...")
    enriched = []
    ...
    for i, sig in enumerate(signals, 1):
        ...
        entry_bar = None; entry_table = None
        for (ot,) in candidates:
            entry_bar, entry_table = fetch_chain_row_smart(
                sb, sym, int(strike), ot, str(expiry), sig_ts)
            if entry_bar:
                conv = "CE/PE" if ot in ("CE", "PE") else "CALL/PUT"
                if option_type_convention is None:
                    option_type_convention = conv
                    print(f"      [probe] option_type convention = {conv}")
                break

        if entry_bar is None:
            skipped_no_entry += 1
            continue
        table_hits[f"entry:{entry_table}"] += 1

        ot_final = ("CE" if act == "BUY_CE" else "PE") if option_type_convention == "CE/PE" else ("CALL" if act == "BUY_CE" else "PUT")
        exit_bar, exit_table = fetch_chain_row_smart(
            sb, sym, int(strike), ot_final, str(expiry),
            sig_ts + timedelta(minutes=EXIT_OFFSET_MIN))
        if exit_bar is None:
            skipped_no_exit += 1
            continue
        table_hits[f"exit:{exit_table}"] += 1
    ...
    print(f"  Table hits: {dict(table_hits)}")
    ...
    if not enriched:
        print("*** No enriched signals — investigate ***")
        return 1
    ...
    print("=" * 100)
    print(f"PATH 1 v3 — LIVE COHORT P&L (8 weeks, T+30m exit, ATM strike, sig-expiry, archive+live)")
    print("=" * 100)
