...
def main() -> None:
    ...
    payload = {
        ...
        "gamma_features": {
            ...
            "flip_distance": gamma_row.get("flip_distance"),
            "flip_distance_pct": gamma_row.get("flip_distance_pct"),
        ...
        "wcb_features": {
            "symbol": symbol,
            "snapshot_ts": wcb_row.get("ts") if wcb_row else None,
            "wcb_score": wcb_row.get("wcb_score") if wcb_row else None,
            "wcb_regime": wcb_row.get("wcb_regime") if wcb_row else None,
            "matched_constituents": to_int(wcb_row.get("constituent_count")) if wcb_row else None,
            "expected_constituents": to_int(wcb_row.get("constituent_count")) if wcb_row else None,
            "matched_weight_pct": matched_weight_pct,
            "missing_symbols": (wcb_row.get("raw", {}).get("missing_intraday", []) if isinstance(wcb_row.get("raw"), dict) else []) if wcb_row else [],
            "phase": "WCB_PHASE_1",
            "weights_mode": "TOP10_OFFICIAL_PLUS_RESIDUAL_PLACEHOLDER",
            "is_partial": (matched_weight_pct is not None and matched_weight_pct < 100.0) if wcb_row else False,
            "source_table": "weighted_constituent_breadth_snapshots" if wcb_row else None,
        } if wcb_row else None,
        "raw": {
            "builder": "build_market_state_snapshot_local.py",
            "builder_version": "MINIMAL_RECOVERY_WCB_FIXED",
    ...
    print(f"Inserted ID:              {inserted.get('id')}")
    print(f"Snapshot TS:              {inserted.get('ts')}")
    print(f"Symbol:                   {inserted.get('symbol')}")
