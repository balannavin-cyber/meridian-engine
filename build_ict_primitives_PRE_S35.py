...
def _load_expiry_calendar(sb, symbol: str) -> list:
    """ENH-106 (S33) v7 — load sorted list[date] of all expiries for symbol.

    Queries hist_atm_option_bars_5m (small 27k-row table) for DISTINCT
    expiry_date. Vendor-identical expiry calendar to hist_option_bars_1m
    but much faster to scan. Returns sorted list of date objects.
    ...
    while True:
        ...
        for r in rows:
            ...
            if isinstance(exp, str):
                expiries.add(date.fromisoformat(exp))
            else:
                expiries.add(exp)
        if len(rows) < PAGE_SIZE:
            break
...
def _prefetch_chain_for_primitives(sb, primitives, events, bars_1m, idx, atm_cache):
    """ENH-106 (S33) v7 — enumerate (strike, expiry, opt_type) tuples needed
    for all formation + retest + event anchors and bulk-fetch chain bars.
    ...
    Strategy:
      ...
      5. Per unique (strike, expiry, opt_type) tuple: range query against
         chain over [min_ts, max_ts+1min) of needed timestamps. Cache by
         minute-floored real-UTC ISO key.

    Populates atm_cache:
      ("expiry_calendar", instrument_id) → sorted list[date]
      ("chain_prem", instrument_id, strike, expiry_str, opt_type)
        → dict {real_utc_iso_minute_floored: close_float}
    """
    ...
    if not calendar:
        log(f"  [v7] empty expiry calendar for {symbol}; skipping chain prefetch")
        return 0
    ...
    if n_tuples == 0:
        return 0
    log(f"  [v7] {symbol}: {n_tuples} (strike,expiry,type) tuples to prefetch "
    ...
    for tup_i, (tup, ts_set) in enumerate(timestamps_per_tuple.items()):
        ...
        min_ts = min(ts_set).replace(second=0, microsecond=0)
        max_ts = max(ts_set).replace(second=0, microsecond=0) + timedelta(minutes=1)
        ...
        while True:
            ...
            try:
                ...
                log(f"  [v7] chain query error tup={tup} page={page}: {ex}")
                break
            rows = res.data or []
            for r in rows:
            ...
            if len(rows) < PAGE_SIZE:
                break
            page += 1
        atm_cache[("chain_prem", instrument_id, strike, expiry_str, opt_type)] = cache_dict
        if (tup_i + 1) % 100 == 0:
            log(f"    [v7] prefetch {tup_i + 1}/{n_tuples} tuples  "
                f"{total} bars  elapsed={_time.time() - t0:.1f}s")
    log(f"  [v7] chain prefetch done: {symbol} {total} bars across {n_tuples} "
        f"tuples in {_time.time() - t0:.1f}s")
    return total
