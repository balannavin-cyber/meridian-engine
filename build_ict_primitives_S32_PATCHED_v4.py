...
def _prefetch_atm_calendar(sb, symbol: str, start_utc: datetime,
    ...
    """ENH-100 (S32) v4 — pre-fetch all hist_atm_option_bars_5m rows for symbol
    over [start_utc, end_utc + ATM_PREMIUM_WINDOW_MIN].

    Eliminates per-primitive DB roundtrips by loading the entire ATM calendar
    into memory once per symbol. Replaces v3's ~5-queries-per-primitive pattern
    (which caused both 26-min outcomes compute and HTTP/2 GOAWAY connection
    termination at ~20k streams).

    Populates two structured atm_cache entries (keyed by tuple to avoid collision
    with per-primitive cache keys):
      ("atm_cal_rows", instrument_id) → dict (bar_ts_iso, expiry_iso) → row
      ("atm_cal_expiries", instrument_id) → dict bar_ts_iso → sorted [expiry_iso, ...]
    ...
    if not instrument_id:
        ...
        return 0
    # +70min buffer so future-horizon (max +60m) lookups near window edge resolve
    fetch_end_utc = end_utc + timedelta(minutes=ATM_PREMIUM_WINDOW_MIN)
    rows_by_key: dict = {}
    expiries_by_ts: dict = {}
    page = 0
    t0 = _time.time()
    while True:
        ...
        res = (
            sb.table("hist_atm_option_bars_5m")
              ...
              .eq("instrument_id", instrument_id)
              .gte("bar_ts", start_utc.isoformat())
              .lte("bar_ts", fetch_end_utc.isoformat())
              .order("bar_ts", desc=False)
              .range(offset, offset + PAGE_SIZE - 1)
              .execute()
        )
        rows = res.data or []
        for r in rows:
            # Canonicalize bar_ts via parse_supabase_ts so the lookup key matches
            # what _atm_anchor_at / _atm_future_at compute via .isoformat() on
            # a tz-aware UTC datetime produced by _floor_5m.
            bts_dt = parse_supabase_ts(r["bar_ts"])
...
def _atm_anchor_at(sb, instrument_id: str, bar_ts_5m: datetime,
    ...
    """ENH-100 (S32) v4 — anchor ATM row at exact 5m bar_ts; nearest-expiry first.

    Fast path: read from prefetched calendar (atm_cache[("atm_cal_rows", ...)]).
    Fallback path: v3 per-query against hist_atm_option_bars_5m (only used when
    prefetch wasn't called — e.g., live mode or partial replay). Per-primitive
    cache still applies on the fallback path.
    """
    ...
    if cal_rows is not None and cal_expiries is not None:
        ...
        if not expiries:
            return None
        nearest = expiries[0]  # sorted ascending — nearest expiry first
        return cal_rows.get((bts_key, nearest))

    # Fallback: per-query (v3 path)
...
def _atm_future_at(sb, instrument_id: str, bar_ts_5m: datetime,
    ...
    """ENH-100 (S32) v4 — future ATM row at exact 5m bar_ts + expiry.

    Fast path: prefetched calendar. Fallback: v3 per-query.
    """
    # Fast path: prefetched calendar
    cal_rows = atm_cache.get(("atm_cal_rows", instrument_id))
