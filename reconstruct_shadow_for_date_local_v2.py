...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: python reconstruct_shadow_for_date_local_v2.py YYYY-MM-DD")
        return 1
    ...
    print("========================================================================")
    print("MERDIAN - reconstruct_shadow_for_date_local_v2")
    print("========================================================================")
    ...
    for market_state in market_states:
        ...
        momentum_v2 = latest_row_within_window(momentum_rows, symbol, ts, max_lookback_minutes=90)
        options_flow = latest_row_within_window(options_flow_rows, symbol, ts, max_lookback_minutes=90)
        iv_context = latest_row_within_window(iv_context_rows, symbol, ts, max_lookback_minutes=1440)
        smdm = latest_row_within_window(smdm_rows, symbol, ts, max_lookback_minutes=90)
    ...
    print("------------------------------------------------------------------------")
    print("Writing rows to public.shadow_reconstruction_v2 ...")
    inserted = supabase_insert("shadow_reconstruction_v2", out_rows)
    print(f"Inserted rows returned by Supabase: {len(inserted)}")
    print("RECONSTRUCT SHADOW FOR DATE V2 COMPLETED")
    return 0
