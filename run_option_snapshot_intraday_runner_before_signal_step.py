...
def append_process_output(stdout_text: str, stderr_text: str) -> None:
    with open(LOG_FILE, "a", encoding="utf-8") as f:
        if stdout_text:
            f.write(stdout_text)
...
def run_command(cmd: list[str]) -> int:
    ...
    append_process_output(result.stdout, result.stderr)
...
def run_ingest(symbol: str) -> str | None:
    ...
    append_process_output(proc.stdout, proc.stderr)
...
def run_breadth_refresh() -> int:
    log("------------------------------------------------------------")
    log("REFRESHING INTRADAY BREADTH")
    log("------------------------------------------------------------")
...
def run_full_cycle() -> None:
    breadth_rc = run_breadth_refresh()
    if breadth_rc != 0:
        log("ERROR: breadth refresh failed for this cycle; skipping symbol pipelines")
        return
...
def main() -> None:
    log("=" * 72)
    log("Gamma Engine - Intraday Options Snapshot Runner")
    log("=" * 72)
