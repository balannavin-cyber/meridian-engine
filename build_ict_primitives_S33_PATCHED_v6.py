...
def compute_retest_atm_pnl(primitive: Primitive, outcome_row: "OutcomeRow",
                           ...
                           sb, atm_cache: dict) -> dict:
    """ENH-103 (S33) v6 — retest-anchored ATM CE/PE PnL at 5 horizons.

    Reads ATM premium at outcome_row.first_retest_ts (anchor) and at +5/+15/+30/+60min
    + EOD (last 5m bar of retest's IST trading day = IST 15:25:00 UTC-relabeled).
    BULL primitives use ce_close; BEAR use pe_close.

    Same-strike enforcement: if vendor's atm_strike differs between anchor and
    horizon, that horizon's column is left NULL (premium-percent change not
    meaningful on a rolled strike).

    Returns dict of populated columns (subset of option_pnl_5m / 15m / 30m / 60m / eod).
    Returns {} when:
      - sb is None (smoke mode)
      - primitive.direction not in ('BULL','BEAR')
      - outcome_row.retest_status != 'RETESTED'
      - outcome_row.first_retest_ts is None
      - vendor lacks coverage at retest anchor
      - anchor premium is NULL or zero

    Reuses v5 prefetched calendar via _atm_anchor_at / _atm_future_at fast paths.
    Zero net DB queries when called from backfill (calendar pre-loaded per symbol).
    """
    out: dict = {}
    if sb is None or primitive.direction not in ("BULL", "BEAR"):
        return out
    ...
    anchor_row = _atm_anchor_at(sb, instrument_id, retest_5m, atm_cache)
    if anchor_row is None:
        return out
    ...
    anchor_strike_raw = anchor_row.get("atm_strike")
    anchor_expiry = anchor_row.get("expiry_date")
    if anchor_strike_raw is None or anchor_expiry is None:
        return out
    try:
        anchor_strike = float(anchor_strike_raw)
    except (ValueError, TypeError):
        return out

    def _resolve_future(future_5m: datetime) -> Optional[float]:
        """Returns future_premium if vendor has the bar AND strike is unchanged; else None."""
        future_row = _atm_future_at(sb, instrument_id, future_5m, anchor_expiry, atm_cache)
        if future_row is None:
            return None
        future_strike_raw = future_row.get("atm_strike")
        if future_strike_raw is None:
            return None
        try:
            if float(future_strike_raw) != anchor_strike:
                return None  # strike rolled
        except (ValueError, TypeError):
            return None
        fp_raw = future_row.get(premium_col)
        if fp_raw is None:
            return None
        try:
            fp = float(fp_raw)
    ...
    # Forward horizons: 5 / 15 / 30 / 60 minutes from retest anchor
    for col, mins in [("option_pnl_5m", 5), ("option_pnl_15m", 15),
                       ("option_pnl_30m", 30), ("option_pnl_60m", 60)]:
        future_premium = _resolve_future(retest_5m + timedelta(minutes=mins))
        if future_premium is not None:
            out[col] = (future_premium - premium_t0) / premium_t0 * 100.0

    # EOD horizon: last 5m bar of retest's IST trading day.
    # IST 15:30 is market close; the last 5m bar covers [15:25, 15:30) with bar_ts = 15:25:00.
    ...
    if eod_5m > retest_5m:  # only meaningful if retest is BEFORE the EOD bar
        future_premium = _resolve_future(eod_5m)
        if future_premium is not None:
