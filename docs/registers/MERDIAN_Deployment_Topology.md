...
```
- 10:40 UTC = 16:10 IST, after the 10:00 UTC NSE close and the 03:05 UTC token refresh.
- `run_equity_eod_until_done.py` is the **EOD chain runner** — it loops ingest (`ingest_equity_eod_local.py` → `equity_eod`) → build (`build_breadth_indicators_daily_local.py` → `breadth_indicators_daily`) → coverage (`coverage_check.py`), cursored 50 tickers/lap via `breadth_ingest_state`, until the ~1,385-ticker universe drains.
- The cron **existed all along** (runbook L437 / reference.json) and the **cron daemon fires** (S67 sentinel test: a 1-minute `cron-alive` sentinel landed within the minute). The freeze was NOT a missing cron — the loop **self-aborted** at lap 1 on the builder's cold-day 95%-gate rc=1 (L182 break), which the S67 fix made non-fatal. See System Map §S67 for the full chain-of-causation.

**Load-once-global hazard (TD-S66-NEW-1) — CLOSED.** `ingest_equity_eod_local.py` now reads the Dhan token at USE (`_current_dhan_token()` re-loads `.env` per fetch), not at import. NOTE the AWS scheduled path was already blunted because the runner re-execs the ingest as a fresh subprocess each lap (each lap fresh-reads `.env`); the fix additionally protects any single-process caller. Verified live (RELIANCE auth 200). The §5.1 Dhan token dual-writer flow (`pull_token_from_supabase.py` daily + `refresh_dhan_token.py` 03:05) is unchanged and sound.

**Probe coverage gap (S66) still open** — the daily token probe still doesn't cover `/v2/charts/historical`; unchanged this session (endpoint was healthy).
...
**Why it lapsed is the part worth carrying.** The S71 resolution rested on §S71.4's ~17 MB/day measurement, which was **correct for its regime** and was invalidated by a consumer installed 2026-09-06 (**D.37.8**). **A resolved item has no watcher** — when its premise expired, nothing fired. That is the argument for item 2 above: discipline does not reopen a closed item whose premise has changed; only a live check does. Whether 2026-09-22 is the second or third failure of its class is **not settled**: two EBS exhaustions are documented (08-12, 09-22), §S71.4 was pressure without outage, and 2026-05-14's 62 GB was **Supabase `market_ticks` — a different resource on a different volume** (**D.37.10**).
