...
| **Mechanism** | **Read this session — `compute_gamma_metrics_local.py:338`.** With `spot` supplied (production always supplies it) the function builds a cumulative sum of per-strike gamma from the **minimum strike upward**, then branches on regime. **`net_gex >= 0` (LONG):** walk **outward from the ATM index in both directions**, take the first cumulative sign change found descending and the first found ascending, and return a **linearly interpolated price between the two bracketing strikes** (`strike_prev + frac * (strike_curr - strike_prev)`) — the crossing **nearest spot**, not the first crossing. **`net_gex < 0` (SHORT):** the cumulative array is **abandoned entirely** in favour of `_flip_short_gamma_perstrike(strikes, strike_map, spot)`, a **per-strike sign boundary nearest spot** (TD-S62-NEW / S63). **`spot` absent or non-positive:** legacy fallback — the bottom-up walk returning the **first** low-to-high crossing, which is the naive construction. **UNREAD, not absent:** `_flip_short_gamma_perstrike`'s body was not opened, and the LONG path's `return` lies past `:424` and was not seen — the two directional loops were read; the statement selecting among `candidates` was not. |
...
### TD-S79-NEW-20 (S2 priority) — `compute_flip_level` is defined in five files in at least three distinct shapes, and the replay copy's signature carries no `spot` — byte-identical to the text S27 replaced

| Field | Value |
|---|---|
| **Priority** | **S2.** If it holds, every historical or what-if study that read a flip through the replay path ran against a construction that is not the live one. **Signature divergence VERIFIED S79 from a source read; the replay body remains UNREAD** — which is why S2 and not S1. |
| **Filed** | 2026-09-16 (Session 79). **Amended same session** from a source read of all seven `grep` hits — signatures only, no bodies. |
...
| **Symptom (VERIFIED S79 — signatures read from source; no body read)** | The five definitions carry **three distinct shapes**: (a) `(strike_map: dict[float, float], spot: float \| None = None) -> float \| None` — `compute_gamma_metrics_local.py:338`, the live one, `spot` optional; (b) `(strike_map, spot)` untyped with `spot` **required** — `measure_rate_sensitivity.py:121` and `backfill_hist_greeks.py:170`; (c) `(rows, spot)` — `backfill_gamma_metrics.py:86`, which takes **records rather than a strike map** and builds its own. Replay is a fourth variant of shape (a) with the parameter removed: `(strike_map: dict[float, float]) -> float \| None` — **no `spot` at all**. |
| **Primary evidence — byte identity (replaces the grep-shape inference the entry first rested on)** | Replay's signature at `:233` is **byte-identical** to `OLD_COMPUTE_FLIP` at `fix_td_new_2_flip_level.py:84` — the exact text the S27 patch replaced. Both read `def compute_flip_level(strike_map: dict[float, float]) -> float \| None:`. That is consistent with **replay having been missed by that patch**, and it is stronger than the shape inference it supersedes. It is **not proof the body was never separately changed**: identical signatures place no constraint on bodies, and replay's body was not read. |
| **What is claimed, and what is NOT** | **Claimed:** five files define the function; three distinct parameter shapes exist among them; the replay signature has no `spot` and therefore **cannot take one**; that signature matches the pre-S27 text byte-for-byte. **Not claimed:** that the replay body executes the legacy walk. **Its body was not read**, and the legacy-path conclusion is deliberately *not* drawn from the signature — a body deriving an ATM by some other means, or one separately amended after S27, would change it. |
| **Correction to the original filing (mine)** | The entry as first written said `compute_flip_level` "exists in **seven** places". That was a **`grep` hit count filed as a copy count**: two of the seven are string constants inside the S27 patch script, not live definitions. Five files define it. The error was mine, in the S79 filing, and it overstated the live surface by two — the same shape as NEW-17's demoted numbers, one register entry apart. |
| **Why it matters if it holds** | ADR-008 scopes replay as the what-if sandbox **and** as the safety harness for code-dependency migrations — S25 used it to validate the `ret_session` anchor migration *before* a production change. A harness computing a different flip from production **validates the wrong thing silently**, which is the S70 shape: a green run against the wrong artefact is indistinguishable from success. |
...
| **Proper fix** | **Read the body of `replay/replay_compute_gamma_metrics.py:233` and the lines beneath it** — the signature question is settled, the body question is not. If the legacy walk is confirmed, decide whether replay should track production's construction, and whether prior replay-derived flip findings need re-labelling — that decision is ADR-008 territory, not a patch. The three-shape divergence is a **separate** question: whether these five should be one shared function at all. |
| **Cost to fix** | ~5 min to read the replay body. Unknown to remediate; the re-labelling question, and the shared-function question, are each larger than the code change. |
| **Cross-ref** | TD-S79-NEW-17 (what the live construction actually is) · ADR-008 (replay's scope and the harness guarantee) · S72 (flat-namespace hazard) · S70 (green run against the wrong artefact) · S27 `241f943` (the TD-NEW-2 patch whose OLD text replay still matches). |
| **Status** | **OPEN — SIGNATURES VERIFIED FROM SOURCE, BODIES UNREAD.** |
