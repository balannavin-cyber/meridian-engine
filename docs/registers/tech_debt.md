...
### TD-S79-NEW-20 (S2 priority) — `compute_flip_level` exists in seven places and the replay copy carries the pre-`spot` signature, so replay and production may not compute the same flip

| Field | Value |
|---|---|
| **Priority** | **S2.** If it holds, every historical or what-if study that read a flip through the replay path ran against a construction that is not the live one. **INFERRED, NOT VERIFIED** — which is why S2 and not S1. |
| **Filed** | 2026-09-16 (Session 79) |
| **Component** | `replay/replay_compute_gamma_metrics.py:233` vs `compute_gamma_metrics_local.py:338`. Five further copies: `backfill_gamma_metrics.py:86`, `measure_rate_sensitivity.py:121`, `backfill_hist_greeks.py:170`, and two literals inside `fix_td_new_2_flip_level.py` (`:84` OLD, `:115` NEW). |
| **Symptom (INFERRED from a grep signature — no body was read)** | `grep -rn "def compute_flip_level"` returns **seven** definitions. The live one takes `(strike_map, spot=None)`. The replay one takes `(strike_map)` — **no `spot` parameter at all**. Per the live function's own fallback branch, with `spot` absent the only reachable path is the **legacy bottom-up walk returning the first low-to-high crossing**. If the replay body matches what its signature implies, **replay cannot execute either the ATM-outward LONG path or the SHORT-gamma per-strike branch**, and computes a different flip from production on every run where those diverge from the naive one. |
| **What is claimed, and what is NOT** | **Claimed:** seven definitions exist; the replay signature lacks `spot`. **Not claimed:** that the replay body is the legacy walk — **its body was not read.** The signature makes that the only form it *can* take if the parameter is genuinely absent, but a default assigned inside the body, or a different construction entirely, would change the conclusion. |
...
| **Adjacent hazard** | Seven same-named definitions in a flat repo root is the S72 structural finding in miniature — **symbol and filename similarity is actively misleading**, and a reader inferring which `compute_flip_level` is production from its name will pick wrong. |
| **Proper fix** | **A one-line read of `replay/replay_compute_gamma_metrics.py:233` and the lines beneath it**, to confirm or refute the legacy-path inference. If confirmed, decide whether replay should track production's construction, and whether prior replay-derived flip findings need re-labelling — that decision is ADR-008 territory, not a patch. |
| **Cost to fix** | ~5 min to verify. Unknown to remediate; the re-labelling question is larger than the code change. |
| **Cross-ref** | TD-S79-NEW-17 (what the live construction actually is) · ADR-008 (replay's scope and the harness guarantee) · S72 (flat-namespace hazard) · S70 (green run against the wrong artefact). |
| **Status** | **OPEN — INFERRED FROM A SIGNATURE, NOT VERIFIED.** |
