...
MOMENTUM_SOURCE = "momentum_engine_v4_3"
...
def get_previous_momentum_row_same_session(symbol: str, current_ts: str):
    rows = rest_get(
        "momentum_snapshots",
        {
            "symbol": f"eq.{symbol}",
            "select": "ts,session_vwap,price_vs_vwap_pct,vwap_slope,atm_straddle_change,source,created_at",
    ...
    for row in rows:
        row_dt = parse_ts(row.get("ts"))
        if row_dt is None:
            continue
        if row_dt >= current_dt:
            continue
        if row_dt.date() != current_dt.date():
            continue
        return row
...
def build_momentum_row(symbol: str):
    ...
    session_vwap = latest_metrics["straddle_vwap"]
    ...
    previous_momentum = get_previous_momentum_row_same_session(symbol, current_bucket_ts)
    vwap_slope = None
...
def main():
    ...
    for symbol in symbols:
        log(f"Building momentum v4.3 for {symbol} ...")
