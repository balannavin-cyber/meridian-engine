...
def _atm_strike_for(spot: float, symbol: str) -> Optional[float]:
    """Nearest strike to spot, lower-on-tie. ENH-100 (S32)."""
    interval = STRIKE_INTERVAL.get(symbol)
    if not interval or interval <= 0:
        return None
    lower = (spot // interval) * interval
    upper = lower + interval
    # tie → lower (operational convention; matches per-ADR-010 v3 spec)
    return lower if (spot - lower) <= (upper - spot) else upper


def _nearest_expiry_for(sb, instrument_id: str, on_or_after: date,
                        atm_cache: dict) -> Optional[date]:
    """ENH-100 (S32) v2 — nearest vendor-available expiry on or after a date.

    Cached per (instrument_id, on_or_after) so multiple primitives on the same
    session date reuse one query. Returns None if no expiry coverage.

    v2 (S32 hotfix): added trade_date anchor predicate to avoid Postgrest 57014
    statement timeout on multi-million-row hist_option_bars_1m. trade_date is an
    indexed date column; 7-day forward window absorbs typical Indian market
    holiday gaps. OI-18 anti-pattern class fix (unbounded ORDER BY+LIMIT scan).
    """
    ck = ("expiry_lookup", instrument_id, on_or_after.isoformat())
    if ck in atm_cache:
        return atm_cache[ck]
    window_end = on_or_after + timedelta(days=7)
    res = (
        sb.table("hist_option_bars_1m")
          .select("expiry_date")
          .eq("instrument_id", instrument_id)
          .gte("trade_date", on_or_after.isoformat())
          .lt("trade_date", window_end.isoformat())
          .gte("expiry_date", on_or_after.isoformat())
    ...
    if not rows:
        atm_cache[ck] = None
        return None
    raw = rows[0]["expiry_date"]
    exp = date.fromisoformat(raw) if isinstance(raw, str) else raw
    atm_cache[ck] = exp
    return exp


def _option_bars_for_contract(sb, instrument_id: str, expiry_date_v: date,
                              strike: float, option_type: str,
                              window_start: datetime, window_end: datetime,
                              atm_cache: dict) -> list[tuple[datetime, float]]:
    """ENH-100 (S32) — option close bars for one contract over a window.

    Cached per (instrument_id, expiry, strike, option_type, valid_from_minute_floor)
    so a primitive that lands on the same minute as a sibling reuses one query.
    Returns [(ts_utc, close), ...] sorted ascending.
    """
    ck = (
        "opt_bars", instrument_id, expiry_date_v.isoformat(),
    ...
    res = (
        sb.table("hist_option_bars_1m")
          .select("bar_ts,close")
          .eq("instrument_id", instrument_id)
          .eq("expiry_date", expiry_date_v.isoformat())
          .eq("strike", float(strike))
          .eq("option_type", option_type)
          .gte("bar_ts", window_start.isoformat())
          .lte("bar_ts", window_end.isoformat())
          .order("bar_ts", desc=False)
          .execute()
    )
    rows = res.data or []
    out = [(parse_supabase_ts(r["bar_ts"]), float(r["close"])) for r in rows]
    atm_cache[ck] = out
    return out
...
def compute_atm_pnl_and_dte(primitive: Primitive, bars_1m: list[Bar],
    ...
    """ENH-100 (S32) — formation-anchored ATM CE/PE PnL at 4 horizons + DTE.

    Skipped when:
      - sb is None (smoke test mode)
      - primitive.direction not in {BULL, BEAR} (no option type defined)
      - symbol unknown (no instrument_id)
      - spot at valid_from unavailable
      - no vendor expiry coverage
      - no premium bars for the chosen (expiry, strike, option_type)

    Same-strike tracking: strike identified at +0 followed through +5/+15/+30/+60.
    No re-pick at later horizons.
    """
    out: dict = {}
    ...
    if not instrument_id:
        return out
    spot_t0 = _spot_at(bars_1m, primitive.valid_from, idx)
    if spot_t0 is None or spot_t0 == 0:
        return out
    atm_strike = _atm_strike_for(spot_t0, primitive.symbol)
    if atm_strike is None:
        return out

    valid_from_date = primitive.valid_from.astimezone(IST).date()
    nearest_expiry = _nearest_expiry_for(sb, instrument_id, valid_from_date, atm_cache)
    if nearest_expiry is None:
        return out
    out["dte_at_formation"] = (nearest_expiry - valid_from_date).days

    option_type = "CE" if primitive.direction == "BULL" else "PE"
    window_start = primitive.valid_from - timedelta(minutes=2)
    window_end = primitive.valid_from + timedelta(minutes=ATM_PREMIUM_WINDOW_MIN)
    opt_bars = _option_bars_for_contract(
        sb, instrument_id, nearest_expiry, atm_strike, option_type,
        window_start, window_end, atm_cache,
    )
    if not opt_bars:
        return out
    opt_idx = {ts.replace(second=0, microsecond=0): close for ts, close in opt_bars}

    premium_t0 = _option_at(opt_idx, primitive.valid_from)
    if premium_t0 is None or premium_t0 == 0:
        return out
    for mins in ATM_PNL_WINDOWS_MIN:
        future_ts = primitive.valid_from + timedelta(minutes=mins)
        p = _option_at(opt_idx, future_ts)
        if p is not None:
            out[f"atm_pnl_{mins}m_pct"] = (p - premium_t0) / premium_t0 * 100.0
    return out
