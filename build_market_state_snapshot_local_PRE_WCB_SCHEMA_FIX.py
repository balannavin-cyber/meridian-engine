...
This version adds:
- wcb_features JSON block in market_state_snapshots
...
def pick_table_row(
    ...
    """
    Try candidate tables in order and return the first table with data.
    """
    ...
    if last_error:
        print(f"[WARN] No data found in candidate tables {table_candidates}. Last error: {last_error}")
    else:
        print(f"[WARN] No data found in candidate tables {table_candidates}.")
    return "", None
...
def pick_global_row(
    ...
    """
    For tables that may not be keyed by symbol, try fetching latest row.
    """
    filters = {}
    if extra_filters:
        filters.update(extra_filters)
...
def build_wcb_features(symbol: str, wcb_row: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    matched_rows = to_int(first_non_empty(wcb_row, ["matched_rows", "matched_constituents", "matched_intraday"]))
    active_rows = to_int(first_non_empty(wcb_row, ["active_rows", "expected_constituents", "basket_size"]))
    matched_weight_pct = to_float(
        first_non_empty(
            wcb_row,
            [
                "matched_weight_pct",
                "matched_weight_coverage_pct",
                "coverage_pct",
                "weight_coverage_pct",
            ],
        )
    ...
    missing_symbols = first_non_empty(wcb_row, ["missing_symbols", "missing_tickers", "missing"])
    if isinstance(missing_symbols, str):
        missing_symbols = [x.strip() for x in missing_symbols.split(",") if x.strip()]
    if not isinstance(missing_symbols, list):
        missing_symbols = []

    weights_mode = first_non_empty(wcb_row, ["weights_mode", "weight_mode"])
    if not weights_mode:
        weights_mode = "TOP10_OFFICIAL_PLUS_RESIDUAL_PLACEHOLDER"

    regime = normalize_wcb_regime(first_non_empty(wcb_row, ["wcb_regime", "regime"]))
    score = to_float(first_non_empty(wcb_row, ["wcb_score", "index_breadth_score", "score"]))
    ...
    snapshot_ts = first_non_empty(wcb_row, ["ts", "created_at"])
    ...
    return clean_json(
        {
            ...
            "wcb_score": score,
            "wcb_regime": regime,
            "matched_constituents": matched_rows,
            "expected_constituents": active_rows,
    ...
    return clean_json(
        {
            ...
            "builder_version": "WCB_MEASUREMENT_V1",
...
def fetch_latest_gamma(client: SupabaseRestClient, symbol: str) -> Tuple[str, Dict[str, Any]]:
    candidates = ["gamma_metrics"]
    table, row = pick_table_row(client, candidates, symbol)
    if not row:
...
def fetch_latest_volatility(client: SupabaseRestClient, symbol: str) -> Tuple[str, Dict[str, Any]]:
    candidates = ["volatility_snapshots"]
    table, row = pick_table_row(client, candidates, symbol)
    if not row:
...
def fetch_latest_momentum(client: SupabaseRestClient, symbol: str) -> Tuple[str, Dict[str, Any]]:
    candidates = ["momentum_features", "momentum_snapshots"]
    table, row = pick_table_row(client, candidates, symbol)
    if not row:
...
def fetch_latest_breadth(client: SupabaseRestClient, symbol: str) -> Tuple[str, Dict[str, Any]]:
    """
    Equal breadth can live in different tables depending on how the current MERDIAN schema is organized.
    We try the most likely candidates in a safe order.
    """
    symbol_candidates = [
        "latest_market_breadth_intraday",
        "market_breadth_intraday",
        "market_breadth_daily_nse",
        "latest_market_breadth_daily_nse",
    ]

    table, row = pick_table_row(client, symbol_candidates, symbol)
    if row:
        return table, row
    ...
    table, row = pick_global_row(client, global_candidates)
    if row:
        return table, row

    raise RuntimeError(f"No breadth row found for symbol={symbol}")


def fetch_latest_wcb(client: SupabaseRestClient, symbol: str) -> Optional[Dict[str, Any]]:
    candidates = ["weighted_constituent_breadth_snapshots"]
    table, row = pick_table_row(client, candidates, symbol)
    if row:
        return row
    ...
    breadth_features = clean_json(
        {
            ...
            "universe_count": to_int(first_non_empty(breadth_row, ["universe_count", "active_universe"])),
...
def main() -> None:
    ...
    breadth_table, breadth_row = fetch_latest_breadth(client, symbol)
