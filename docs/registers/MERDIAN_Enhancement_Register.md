...
| ENH-07 | Basis-Implied Risk-Free Rate | 1 | **CLOSED (no-op, flat r — S62)** |
...
| ENH-116 | Ambient Environment Intelligence (Regime-Aware Context Layer) | context | **PROPOSED (P2)** |
...
| Status | **CLOSED (no-op, flat r — S62)** (was IN PROGRESS S57 / PROPOSED). **S62 ruling:** ruled a no-op on two grounds — (1) *empirical*: a flat r=6.5% Black-Scholes reconstruction validated against live `gamma_metrics` (sign 99% / magnitude 0.96× / sign-regime 95%), so the basis-implied rate is immaterial to the GEX read; (2) *architectural*: the live gamma engine reads vendor (Dhan) Greeks with no rate injection, so there is no live rate parameter to correct — the rate only ever mattered inside the historical Greeks backfill (TD-S58-NEW-1), which solves at flat r=6.5%. `core/bs_engine.py` (built S61) is that backfill's substrate. The S57 cost-of-carry / basis-velocity sign-read shipped separately as **ENH-07 B** (`basis_context_snapshots`, display-only, S61). **S57 scope add:** cost-of-carry / basis-velocity sign-read (→ ENH-07 B, SHIPPED-Measure S61).
...
### ENH-115 — FII/DII Participant Positioning + Cash Flow (PROPOSED — S57)
...
### ENH-116 — Ambient Environment Intelligence (Regime-Aware Context Layer) (PROPOSED — S62)

| Field | Detail |
|---|---|
| Status | **PROPOSED (P2 — build-deferred, substrate-gated)** |
| Filed | 2026-07-01 (Session 62) |
| Tier | context layer (display-not-gate per ADR-002 v2 GEX-as-context framing) |
| Priority | **P2.** BLOCKED-BY: SENSEX historical backfill completion (S62, bar TD-S62-NEW-2) + `expiry_outcomes` retroactive seed. Sequenced AFTER the S63 doc-close (does not jump the documentation queue). Not P0/P1 (display-not-gate, touches no live routing); not P3 (the enabling substrate was built this session). |
| One-line | The same live structural read (a long-γ pin at range) means opposite things in different environments; MERDIAN reads it unconditionally today. ENH-116 gives it the ambient prior — a reconciled verdict from four regime lenses plus a memory of how past expiries resolved under conditions like today's. |
| Architecture | **Compute at rest, relate at open.** A *post-market compiler* (`compile_market_environment_local.py`) computes the full ambient state from now-complete data and writes tomorrow's `market_environment_snapshots` row (+ appends the labeled `expiry_outcomes` event on expiry days); a *light pre-market reconciler* (`relate_ambient_to_open_local.py`) joins last night's ambient row to this morning's opening structure → the session prior. Three clocks: Positioning Regime (monthly), Cycle-So-Far (weekly), Session Open (intraday, unchanged — now *interpreted against* the slow clocks). |
| Four regime lenses | (1) **Gamma-positioning** — GEX regime persistence, max-γ drift, `gamma_concentration` daily first-derivative (the S62-backfilled column). (2) **Breadth-trajectory** (operator-elevated) — WCB / %>DMA / A/D slopes; the price-vs-breadth divergence is a top-tier tell independent of gamma (gamma = mechanical cage; breadth = fundamental pressure; divergence = the pin holds until it violently doesn't). (3) **Cycle-OI / participant** — call/put OI build asymmetry + ENH-115 FII/DII/Pro deltas (daily, never intraday). (4) **Macro** (operator-added, extensible) — USDINR / crude / gold daily tilt; reserved slots for rates/global. Each lens' pairwise divergences are first-class flagged fields (ADR-017 P4 confluence-is-bidirectional generalized cross-cycle). |
| Expiry memory (3 phases, N-gated) | (A) `expiry_outcomes` labeled event-store NOW — one row per weekly/monthly expiry (ambient state going in + outcome), seeded retroactively from the ~14-month S62-backfilled history; Measure-layer, stores not predicts. (B) conditional base rates LATER — "past expiries matching today held 61% / broke down 31% / broke up 8%, median −0.6% from open pin"; interpretable, degrades gracefully at small N. (C) analog retrieval MAYBE-NEVER — nearest-neighbour over the ambient feature vector, operator-facing, never gates (the ENH-97 Phase-0b FAIL is the discipline anchor). Principle: store the labeled event now, extract intelligence later, never let extraction outrun sample size. |
| View / Console (ADR-017) | Three tiers: **T1 Ambient Verdict** (one headline above the live cards — regime + alignment + base rate + break-direction — reframes everything below); **T2 four-lens strip** (divergence-first, not values — alignment vs divergence legible in half a second); **T3 Expiry Memory** (base-rate distribution + a "show analogs" drill-down to the precedent expiries). Salience rule: quiet when the lenses align, loud when one diverges. Explicit not-shows: no raw slopes on the face, no intraday flicker of the slow lenses. |
| Schemas (proposed) | `market_environment_snapshots` (post-market compiler output; lens columns for gamma/breadth NOW, participant/macro NULL-until-supervised so it ships with Lenses 1+2 today) + `expiry_outcomes` (labeled event-store). Full DDL in the spec. |
| Validation (M→V→S→P) | Measure: build both writers, seed `expiry_outcomes`, display-only console panel, touches no signal routing. Validate: ADR-009 holdout on the seeded expiry cohort, base rates net-of-costs, each lens independently before its divergence flags are trusted. Shadow: run the ambient prior alongside live signals, log what it *would* have advised. Promote: only at N≥30 per regime cell, net-of-costs, and even then a position-size modifier never a hard gate. First year = a display + a forward cohort. |
| Enabled by | TD-S58-NEW-1 (S62) — the historical GEX + `gamma_concentration` backfill; regime trajectories are not computable without it. This is the ADR-002 v2 P4 "regime has velocity" dimension finally reached, extended across nested timescales. |
| Spec | `docs/decisions/ENH-116-ambient-environment-intelligence.md` (301 ln / 18,762 B). Separate doc + this register row (mirrors the ADR/CASE separate-doc convention). |
| Open questions | Weekly-vs-monthly expiry pooling (leaning pooled-with-feature; monthly N small); macro EOD data source (USDINR/crude/gold); whether to reserve a fifth lens (rates term-structure / global overnight) schema slot now. |
| Related | ADR-002 v2 (P4 regime-velocity, P5 local-vs-aggregate); ADR-017 (P4/P3/P1 console); ADR-018 (supervision); ENH-115 (participant source); ENH-SDM (display-first playbook); TD-S58-NEW-1 (substrate); TD-S62-NEW (flip bug — the ambient layer would label short-γ boards as pin/flip-unreliable). |
...
| 2026-07-01 | **unified** | ENH-07 A **IN PROGRESS → CLOSED (no-op, flat r)**; +ENH-116 Ambient Environment Intelligence PROPOSED (P2); Scope row bumped ENH-01..ENH-112 → ENH-01..ENH-116 | (S62 close — commit pending) | **ENH-07 A CLOSED as a no-op** on two grounds: empirical (flat r=6.5% BS reconstruction validated vs live `gamma_metrics` — sign 99% / magnitude 0.96× / sign-regime 95%; basis-implied rate immaterial) + architectural (the live engine reads vendor Dhan Greeks with no rate injection — no live rate parameter exists; the rate only mattered inside the TD-S58-NEW-1 historical backfill, which solves at flat r=6.5%). Supersedes the S61 REFRAMED/open status. The S57 cost-of-carry / basis-velocity sign-read shipped separately as ENH-07 B (S61). **+ENH-116 Ambient Environment Intelligence PROPOSED (P2)** — Part 1 status row + Part 4 detail block + spec `docs/decisions/ENH-116-ambient-environment-intelligence.md` (301 ln). Post-market compiler / pre-market reconciler; four regime lenses (gamma-positioning / breadth-trajectory / cycle-OI-participant / macro USDINR-crude-gold) with pairwise-divergence flags; three-phase expiry-memory (`expiry_outcomes` event-store → base rates → analog retrieval, never gates); three-tier View/Console (Verdict headline / divergence-first lens strip / expiry-memory + show-analogs); two schemas (`market_environment_snapshots` + `expiry_outcomes`); M→V→S→P, display-not-gate first year. BLOCKED-BY SENSEX backfill + expiry-memory seed; enabled specifically by the S62 TD-S58-NEW-1 backfill. Grounded in ADR-002 v2 P4/P5, ADR-017 P4/P3/P1, ADR-018, ENH-115, ENH-SDM. **TD-S58-NEW-1 RESOLVED** (historical per-strike Greeks sidecar + the DISCOVERY that `hist_gamma_metrics` already existed needing only its `gamma_concentration` column filled; NIFTY + SENSEX run to completion bar 2026-01-19 → TD-S62-NEW-2). 0 new ADR (ENH-116 is a PROPOSED spec; ENH-07 A close is a no-op ruling — Doc Protocol v4 Rule 10 bar not met). 0 git commits (backfill scripts + spec staged, carry to S63). Scope row bumped ENH-01..ENH-116. |
