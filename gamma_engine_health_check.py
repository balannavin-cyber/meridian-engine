...
def _session_bounds_ist(now_ist: datetime):
    premarket_start = now_ist.replace(
        hour=PREMARKET_START_HOUR,
        minute=PREMARKET_START_MINUTE,
        second=0,
        microsecond=0,
    )
    ...
    return premarket_start, market_open, market_close


def _session_mode(now_ist: datetime) -> str:
    premarket_start, market_open, market_close = _session_bounds_ist(now_ist)
    current_t = now_ist.time()

    if current_t < premarket_start.time():
        return "Market Closed"
    if current_t < market_open.time():
        return "PREMARKET"
    if current_t <= market_close.time():
        return "Market OPEN"
    return "Market Closed"


def _is_market_open(session_mode: str) -> bool:
    return session_mode == "Market OPEN"
...
def _stage_status(
    ...
    if not _is_market_open(session_mode):
        return "OK"
...
def _pipeline_problems_for_symbol(
    ...
    for table in PIPELINE_TABLES:
        ...
        if previous_ts is not None and current_ts is not None:
            ...
            if _is_market_open(session_mode) and lag > MAX_STAGE_LAG_SECONDS:
                problems.append(
...
def _cross_symbol_problems(
    ...
    if _is_market_open(session_mode):
...
def _pipeline_status_from_worst(nifty_worst: str, sensex_worst: str) -> str:
    ranks = [STATUS_RANK.get(nifty_worst, 3), STATUS_RANK.get(sensex_worst, 3)]
    worst_rank = max(ranks)
    if worst_rank == STATUS_RANK["OK"]:
        return "HEALTHY"
    if worst_rank == STATUS_RANK["STALE"]:
        return "STALE"
    if worst_rank == STATUS_RANK["MISSING"]:
        return "BROKEN"
    return "UNKNOWN"


def _symbol_sync_status(signal_gap: Optional[float], session_mode: str) -> str:
    if signal_gap is None:
        return "UNKNOWN"
    if not _is_market_open(session_mode):
        return "OK"
    if signal_gap <= MAX_SYMBOL_TS_GAP_SECONDS:
        return "OK"
    return "DRIFT"


def _final_verdict_line(
    overall_status: str,
    pipeline_status: str,
    symbol_sync_status: str,
    session_mode: str,
) -> str:
    return (
        f"ENGINE: {overall_status} | "
        f"PIPELINE: {pipeline_status} | "
        f"SYMBOL_SYNC: {symbol_sync_status} | "
...
def main():
    ...
    premarket_start_ist, market_open_ist, market_close_ist = _session_bounds_ist(now_ist)
    ...
    pipeline_status = _pipeline_status_from_worst(worst_stage_nifty, worst_stage_sensex)
    symbol_sync_status = _symbol_sync_status(signal_gap, session_mode)
    final_verdict = _final_verdict_line(
        overall_status=status,
        pipeline_status=pipeline_status,
        symbol_sync_status=symbol_sync_status,
    ...
    print(f"Premarket start:     {premarket_start_ist.strftime('%H:%M:%S IST')}")
    print(f"Market open:         {market_open_ist.strftime('%H:%M:%S IST')}")
    print(f"Market close:        {market_close_ist.strftime('%H:%M:%S IST')}")
