...
def _classify_premium_state(
    ...
    if expand_points >= compress_points + 2 and expand_points >= 3:
        return "EXPANDING"
    if compress_points >= expand_points + 2 and compress_points >= 3:
        return "COMPRESSING"
    return "FLAT"


def _classify_directional_edge_state(
    direction_bias: str,
...
def _classify_signal_mode(
    premium_state: str,
    directional_edge_state: str,
) -> str:
    if directional_edge_state == "VALID":
        return "DIRECTIONAL"
...
def _base_confidence(
    ...
    if premium_state == "EXPANDING":
        score += 2.0
    elif premium_state == "COMPRESSING":
        score -= 6.0
    else:
        score -= 1.0
...
def _apply_premium_state_context(
    score: float,
    reasons: List[str],
    cautions: List[str],
    *,
    ...
    if premium_state == "EXPANDING":
        reasons.append("Premium state is EXPANDING")
        if directional_edge_state == "BLOCKED":
            cautions.append("Premium is expanding but directional edge is blocked")
            score -= 2.0
    elif premium_state == "COMPRESSING":
        cautions.append("Premium state is COMPRESSING")
        score -= 4.0
    else:
        cautions.append("Premium state is FLAT")
        score -= 1.0

    if directional_edge_state == "VALID":
        reasons.append("Directional edge state is VALID")
        score += 2.0
    else:
        cautions.append("Directional edge state is BLOCKED")

    return score


def _apply_signal_mode_context(
    score: float,
    reasons: List[str],
    cautions: List[str],
    *,
    ...
    if signal_mode == "DIRECTIONAL":
        reasons.append("Signal mode is DIRECTIONAL")
        score += 2.0
    elif signal_mode == "EXPANSION_ONLY":
        reasons.append("Signal mode is EXPANSION_ONLY")
        cautions.append("Expansion-only mode is informational, not directional")
        score -= 1.0
    else:
        cautions.append("Signal mode is BLOCKED")
        score -= 2.0
...
def _build_signal(symbol: str) -> Dict[str, Any]:
    ...
    premium_state = _classify_premium_state(
        ret_5m=ret_5m,
        ret_15m=ret_15m,
        ret_30m=ret_30m,
        price_vs_vwap_pct=price_vs_vwap_pct,
    ...
    directional_edge_state = _classify_directional_edge_state(
    ...
    signal_mode = _classify_signal_mode(
        premium_state=premium_state,
        directional_edge_state=directional_edge_state,
    )
    ...
    reasons.append(f"Premium state is {premium_state}")
    reasons.append(f"Directional edge state is {directional_edge_state}")
    reasons.append(f"Signal mode is {signal_mode}")
    if direction_bias != "NEUTRAL":
    ...
    score = _apply_premium_state_context(
        score=score,
        reasons=reasons,
        cautions=cautions,
        premium_state=premium_state,
    ...
    score = _apply_signal_mode_context(
        score=score,
        reasons=reasons,
        cautions=cautions,
    ...
    trade_allowed = directional_edge_state == "VALID"
    ...
    print(f"Derived premium_state (not persisted yet): {premium_state}")
    print(f"Derived directional_edge_state (not persisted yet): {directional_edge_state}")
    print(f"Derived signal_mode (not persisted yet): {signal_mode}")
        ...
        json.dump(
            {
                **signal_row,
                ...
                "premium_state_debug": premium_state,
                "directional_edge_state_debug": directional_edge_state,
                "signal_mode_debug": signal_mode,
            },
            f,
            indent=2,
            default=str,
