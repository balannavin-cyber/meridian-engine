...
def compute_max_gamma_strike(strike_map: dict[float, float]) -> float | None:
    """S41 -- strike with maximum |gex| concentration. None if map empty.

    Domain note: gex_cr is signed (CE positive, PE negative); concentration
    measures absolute magnitude not direction. Tie-break: lower strike wins
    (sorted ascending key iteration; max() returns first-seen on tie).
    """
    if not strike_map:
        return None
    return max(strike_map.items(), key=lambda kv: abs(kv[1]))[0]


def fetch_recent_max_gamma_strikes(
    symbol: str, current_ts: str, n: int = 5
) -> list[float]:
    """S41 -- fetch max_gamma_strike for the last `n` cycles BEFORE current_ts.

    Reads `gex_strike_snapshots` (populated by ENH-80 (S37) writer in this
    same script via build_gss_rows + upsert_gex_strike_snapshots). Per-cycle
    argmax(|gex_cr|) is computed in Python after a bounded Supabase fetch.
    ...
    try:
        ...
        if not rows:
            return []
        # Group by ts, pick argmax(|gex_cr|) per group, return last n in time order.
        ...
        for ts_key in sorted_ts:
            ...
            top = max(strikes_in_cycle, key=lambda r: abs(to_float(r.get("gex_cr"))))
